...
import { X, Eye, EyeOff, Crown, ShieldAlert, Lock } from 'lucide-react'
          ...
          <div className={cn('rounded-3xl bg-gradient-to-br p-6 text-white shadow-2xl', c.gradient)}>
            <div className="flex items-start justify-between">
              ...
              <div className="text-right">
                ...
                <div className="mt-1 text-2xl font-bold opacity-90">{card.hanzi}</div>
              </div>
            </div>
            ...
            {card.teamKind === 'qin' && (
              <div className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-white/20 px-3 py-1 text-sm font-bold">
                <ShieldAlert size={15} /> ENVAHISSEUR
              </div>
            )}
          </div>
...
function SecretBody({ card }: { card: PlayerCard }) {
  if (card.teamKind === 'emperor') {
    return (
      <>
        <p className="text-slate-200">Tu es <b>au-dessus des royaumes</b>. Le Ciel veille.</p>
        <p className="text-slate-400">Tu ne connais personne. Les chefs des royaumes, eux, te connaissent.</p>
      </>
    )
  }
  if (card.teamKind === 'qin') {
    return (
      <>
        <p className="text-slate-200">Tu fais partie de <b>l’Empire Qin</b>, les envahisseurs.</p>
        <p className="text-slate-400">Vous vous connaissez tous entre vous :</p>
        {card.knowsAll && card.knowsAll.length > 0 ? (
          ...
          <p className="text-slate-500">Tu es le seul membre du Qin.</p>
        )}
      </>
    )
  }
  ...
  if (card.isChief) {
    return (
      <>
        <p className="text-slate-200">Tu es le <b>chef</b> de ton royaume.</p>
        {card.knowsEmperor
        ...
        <p className="text-slate-400">Tu ne connais <b>personne d’autre</b> dans ton royaume.</p>
      </>
    )
  }
  return (
    <>
      <p className="text-slate-200">Tu es un membre du royaume.</p>
      <p className="text-slate-200">Tu connais : <b className="text-white text-lg">{card.knowsName}</b></p>
      <p className="text-slate-500 text-sm">(cette personne est dans ton royaume — mais elle ne te connaît pas forcément.)</p>
    </>
  )
}
